...
//********** Managers **********//
export const handleGetAllEntretiens = async (
  request: Request,
  next: NextFunction
) => {
  try {
...
export const handlegetEntretienById = async (
  id: string,
  next: NextFunction
) => {
...
export const handleGetEntretienWithFilters = async (
  request: Request,
  next: NextFunction
) => {
  try {
    ...
    if (request.query.dateEntretien) {
      filters.dateEntretien = new Date(
        request.query.dateEntretien.toString()
      ).toISOString();
    }
    ...
    const entretien = (await EntretienModel.getWithFilters(
      filters
    )) as Entretien[];

    if (entretien.length === 0) {
      throw new Error("Aucun entretien trouvé avec les filtres fournis.");
    }
...
export const handlePostEntretien = async (
  request: Request,
  next: NextFunction
) => {
  try {
    if (
      !request.body.immatriculation ||
      !request.body.idTechnicien ||
      !request.body.typeEntretien ||
      !request.body.dateEntretien ||
      !request.body.note
    ) {
      throw new Error(
        "Les champs immatriculation, idTechnicien, typeEntretien, dateEntretien et note sont requis."
      );
    }
...
export const handleDeleteEntretien = async (
  request: Request,
  next: NextFunction
) => {
  try {
...
export const handlePutEntretien = async (
  request: Request,
  next: NextFunction
) => {
  try {
    ...
    const updatedData: Partial<Entretien> = {
      ...
      typeEntretien: request.body.typeEntretien?.toString(),
      dateEntretien: request.body.dateEntretien
        ? new Date(request.body.dateEntretien)
        : undefined,
      note: request.body.note?.toString() || null,
    };
