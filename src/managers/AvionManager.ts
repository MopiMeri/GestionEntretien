...
//********** Managers **********//
export const handleGetAllAvions = async (
  request: Request,
  next: NextFunction
) => {
...
export const handleGetAvionsWithFilters = async (
  request: Request,
  next: NextFunction
) => {
  try {
    ...
    const avions = (await AvionModel.getWithFilters(filters)) as Avion[];

    if (avions.length === 0) {
      throw new Error("Aucun avion trouvé avec les filtres fournis.");
    }
...
export const handlePostAvion = async (request: Request, next: NextFunction) => {
  try {
    if (
      ...
      throw new Error(
        "Les champs immatriculation, marque et modele sont requis."
      );
    }

    const avionExistante = await AvionModel.getByImmatriculation(
      request.body.immatriculation.toString()
    );
...
export const handleDeleteAvion = async (
  request: Request,
  next: NextFunction
) => {
  try {
    ...
    const avionExist = (await AvionModel.getByImmatriculation(
      immatriculation
    )) as Avion[];

    if (avionExist.length === 0) {
      throw new Error(
        "Immatriculation non trouvée, impossible de supprimer l'avion."
      );
    }
...
export const handlePutAvion = async (request: Request, next: NextFunction) => {
  try {
    ...
    const avionExists = (await AvionModel.getByImmatriculation(
      request.params.immatriculation.toString()
    )) as Avion[];

    if (avionExists.length === 0) {
      throw new Error(
        "Immatriculation non trouvée, impossible de mettre à jour l'avion."
      );
    }
