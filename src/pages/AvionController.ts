...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const avion = await handleDeleteAvion(req, next);
      res.status(200).json({ message: "Avion supprimé avec succès", avion });
    } catch (error) {
      next(error);
    }
  }
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const avion = await handlePutAvion(req, next);
      if (avion) {
        res
          .status(200)
          .json({ message: "Avion mis à jour avec succès", avion });
      }
    } catch (error) {
      next(error);
    }
